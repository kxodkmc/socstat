//! [`StatsExt`] implementation over [`Dataset`]: resolves columns, applies
//! case weights, and forwards to the analysis functions.
...
impl StatsExt for Dataset {
    ...
    fn one_sample_t_test(&self, var: &str, mu0: f64) -> SocStatResult<OneSampleTTest> {
        ...
        onesample::one_sample_t_test(&data, w.as_deref(), mu0)
    }

    fn chi_square_gof_test(&self, var: &str, probs: Option<&[f64]>) -> SocStatResult<ChiSquareGof> {
        let col = self.column_by_name(var)?;
        gof::chi_square_gof(col, self.weights().as_deref(), probs)
    }

    fn mcnemar_test(&self, before_var: &str, after_var: &str) -> SocStatResult<McNemarResult> {
        let c1 = self.column_by_name(before_var)?;
        let c2 = self.column_by_name(after_var)?;
        mcnemar::mcnemar_test(c1, c2, self.weights().as_deref())
    }
    ...
    fn friedman_test(&self, treatment_vars: &[&str]) -> SocStatResult<FriedmanResult> {
        if treatment_vars.len() < 3 {
            return Err(SocStatError::InsufficientData(
                "Friedman test needs at least 3 treatment variables".into(),
            ));
        }
        let cols: Vec<Vec<Option<f64>>> = treatment_vars
            .iter()
            .map(|v| shared::cleaned_numeric_column(self, v))
            .collect::<SocStatResult<_>>()?;
        let weights = self.weights();

        // Listwise-complete rows with a positive weight form the blocks.
        let mut blocks: Vec<Vec<f64>> = Vec::new();
        let mut block_weights: Vec<f64> = Vec::new();
        for i in 0..self.n_rows() {
            let wi = weights.as_ref().map(|ws| ws[i]).unwrap_or(1.0);
            if !wi.is_finite() || wi <= 0.0 {
                continue;
            }
            let row: Option<Vec<f64>> = cols.iter().map(|c| c[i]).collect();
            if let Some(vals) = row {
                blocks.push(vals);
                block_weights.push(wi);
            }
        }
        let refs: Vec<&[f64]> = blocks.iter().map(|b| b.as_slice()).collect();
        let w = if weights.is_some() { Some(block_weights.as_slice()) } else { None };
        friedman::friedman_test(&refs, treatment_vars, w)
    }
    ...
    fn ks_two_sample_test(&self, var1: &str, var2: &str) -> SocStatResult<TwoSampleKsResult> {
        let c1 = shared::cleaned_numeric_column(self, var1)?;
        let c2 = shared::cleaned_numeric_column(self, var2)?;
        let weights = self.weights();
        let (x, wx) = aligned_numeric(&c1, weights.as_deref());
        let (y, wy) = aligned_numeric(&c2, weights.as_deref());
        ks_two_sample::ks_two_sample_test(&x, wx.as_deref(), &y, wy.as_deref())
    }
